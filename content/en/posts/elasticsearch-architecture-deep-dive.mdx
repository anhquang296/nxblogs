...
**Phase 2 — Translog replay**: While phase 1 is running, the primary keeps accepting writes. Those writes are appended to the **translog** (write-ahead log — a pattern where every mutation is recorded to an append-only file **before** it's applied to the actual state, so it can be replayed after a crash or used to sync a replica). After phase 1 completes, the target node "replays" the translog to catch up to the primary. Finally the cluster marks the new shard as active.
