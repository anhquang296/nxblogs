...
To enforce HTTPS only, you add a condition to the bucket policy that denies the request when `aws:SecureTransport` is `false`.

As noted, SSE-C mandates HTTPS already.
...
The policy below denies any `s3:PutObject` that isn't encrypted with SSE-KMS. The condition keys off `s3:x-amz-server-side-encryption` — the header the client uses to declare its encryption type on upload:
...
`StringNotEquals` is true whenever the header isn't `aws:kms` — including when the header is absent entirely — so this one statement blocks both wrong-type uploads and uploads that declare no encryption at all. To pin it down to a specific key, add a condition on the `s3:x-amz-server-side-encryption-aws-kms-key-id` header with that key's ARN.

The bucket policy is checked before Default Encryption applies. That means the policy is the "gatekeeper" — it can stop an invalid request right at the door, while Default Encryption is just a safety net that applies encryption to requests that were already allowed through.
...
While Object Lock protects objects in S3, **Glacier Vault Lock** protects data in the Amazon S3 Glacier cold-storage archive. It also follows the WORM model, but its defining feature is a **lock policy** — once locked, that policy becomes immutable.
