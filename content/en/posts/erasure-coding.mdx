...
description: 'Understand how Erasure Coding protects data with 60% less storage overhead than replication — the math, real-world implementations in S3/HDFS, and trade-offs.'
enableComment: true
---
...
<ExcalidrawDiagram src="/erasure-coding-overview.excalidraw" alt="4+2 configuration splits data into 4 data shards and 2 parity shards" />
...
- **1950s**: Richard Hamming developed <Tooltip content="The first error-correcting codes, capable of detecting and correcting single-bit errors.">Hamming codes</Tooltip>.
- **1960**: Irving Reed and Gustave Solomon published <Tooltip content="The most powerful and widely used erasure coding algorithm to this day.">Reed-Solomon codes</Tooltip>. Originally designed for satellite communications.
...
<Tooltip content="Redundant Array of Independent Disks">RAID</Tooltip> was one of the earliest solutions:
...
**Google File System** (2003) and <Tooltip content="Hadoop Distributed File System">HDFS</Tooltip> defaulted to 3 replicas. The advantages are clear:
...
<ExcalidrawDiagram src="/erasure-coding-simple-parity.excalidraw" alt="Parity by summing 4 data shards, stored across 5 servers" />
...
<ExcalidrawDiagram src="/erasure-coding-reed-solomon.excalidraw" alt="Reed-Solomon treats data shards as polynomial coefficients, evaluated at k+m points" />
...
In practice, Reed-Solomon encoding is implemented using <Tooltip content="The operation of multiplying two arrays of numbers row-by-column to produce a new array.">matrix multiplication</Tooltip>.
...
- The **top k rows** form an <Tooltip content="A matrix with 1s on the diagonal and 0s elsewhere.">identity matrix</Tooltip>. This ensures the first k elements of the output are the original data (unchanged).
- The **bottom m rows** contain the parity coefficients, typically constructed from a <Tooltip content="A matrix where each row consists of powers of a distinct value.">Vandermonde matrix</Tooltip>.

<ExcalidrawDiagram src="/erasure-coding-encoding-matrix.excalidraw" alt="Encoding matrix with identity matrix on top and parity coefficients below" />
...
In section 4.1, addition was presented as a simple abstraction. In practice, the arithmetic doesn't use regular integers or floating-point numbers — it operates in a <Tooltip content="A finite number system named after mathematician Évariste Galois.">Galois Field</Tooltip>.
...
<ExcalidrawDiagram src="/erasure-coding-decoding.excalidraw" alt="Decoding by inverting the submatrix from k surviving shards" />
...
<ExcalidrawDiagram src="/erasure-coding-replication-vs-ec.excalidraw" alt="Storage overhead comparison between replication and erasure coding" />
...
Modern CPUs mitigate this with <Tooltip content="Single Instruction Multiple Data">SIMD instructions</Tooltip>. Intel's ISA-L (Intelligent Storage Acceleration Library) achieves encoding throughput of 2-10 GB/s — fast enough for most workloads, but still notably slower than raw memory copy speeds (20+ GB/s).
...
Erasure coding operates on <Tooltip content="Data blocks that never change in place.">immutable chunks</Tooltip>. If you need to modify a single byte, the entire <Tooltip content="A group of k data shards plus m parity shards, encoded and distributed together.">stripe</Tooltip> (k data shards + m parity shards) must be **re-encoded** — because parity depends on all data shards.

This is why erasure coding is used for <Tooltip content="Write-once, read-many storage.">object storage</Tooltip> like S3, not for **block storage** or **databases** — where data changes constantly.
