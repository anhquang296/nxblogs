...
When a consumer dies and restarts (or when a rebalance occurs in the group), Kafka looks up `__consumer_offsets` to find the last committed offset, then starts reading from the next position. The consumer doesn't need to remember anything — Kafka remembers for it.
