...
<ExcalidrawDiagram src="/kafka-architecture-overview.excalidraw" alt="Kafka Architecture Overview — Producers, Brokers, Partitions, Consumer Groups, KRaft" />
...
The file name is the **base offset** — the offset of the first message in that segment. When a segment reaches its size limit (default 1GB) or time limit, Kafka creates a new segment:
...
When a consumer dies and restarts (or when a rebalance occurs, which means a new consumer join into consumer group), Kafka looks up `__consumer_offsets` to find the last committed offset, then starts reading from the next position. The consumer doesn't need to remember anything — Kafka remembers for it.
...
Disk → Kernel Buffer (copy 1) → User Buffer (copy 2)
→ Socket Buffer (copy 3) → NIC Buffer (copy 4)
...
```
[Zero-copy — 2 copies]
Disk → Kernel Buffer (copy 1) → NIC Buffer (copy 2)
              ↓
    Data NEVER passes through the application
```
