...
description: 'Why opening a new DB connection per request is expensive — TCP handshakes, TLS negotiation, auth overhead — and how connection pooling solves it with pg and RDS Proxy.'
enableComment: true
---
...
This article will dissect exactly what happens when you open and close a database connection, why it's expensive, and how a **connection pool** solves each problem — from a local pool with `pg` in Node.js to a global pool with AWS <Tooltip content="An AWS service that sits between applications and a database, acting as a centralized connection pool. It multiplexes hundreds of client connections into a small number of real database connections, and provides fast failover and IAM authentication.">RDS Proxy</Tooltip>.
...
2. Perform a <Tooltip content="The cryptographic handshake between client and server: exchanging certificates, agreeing on a cipher suite, and creating session keys before any encrypted data can flow.">TLS handshake</Tooltip> to encrypt the channel
...
<ExcalidrawDiagram src="/connection-pool-tcp-handshake.excalidraw" alt="TCP 3-way handshake between client and database costs 1.5 RTT" />

Cost: **1.5 <Tooltip content="Round-Trip Time — the time it takes a packet to travel from client to server and back. This is baseline network latency that cannot be optimized in code; it is bound by physical distance and link quality.">RTT</Tooltip>**. If the application runs in `ap-southeast-1` (Singapore) and the database is in `us-east-1` (Virginia) with ~200ms/RTT latency, you've already lost **300ms** just to establish the TCP connection. Even within the same region, cross-AZ latency of ~1-2ms/RTT adds up quickly when multiplied by thousands of requests.
...
This isn't just time-consuming — it's also **CPU-intensive** — both sides must perform <Tooltip content="Cryptography using public/private key pairs (like RSA or ECDSA) to exchange a session key. Much more CPU-expensive than the symmetric cipher used for the rest of the session.">asymmetric cryptography</Tooltip> to exchange keys.
...
<ExcalidrawDiagram src="/connection-pool-tcp-close.excalidraw" alt="Closing a TCP connection via the 4-way FIN/ACK procedure" />
...
- **Flush <Tooltip content="Translation Lookaside Buffer — a CPU cache that stores recent virtual-to-physical address mappings to speed up memory access.">TLB</Tooltip>** — the cache mapping virtual addresses to physical addresses
...
<ExcalidrawDiagram src="/connection-pool-init-flow.excalidraw" alt="Request borrows an idle connection from the pool then returns it, creating none" />
...
<ExcalidrawDiagram src="/connection-pool-local-problem.excalidraw" alt="N instances each with a local pool, total connections exceed max connections" />
...
<ExcalidrawDiagram src="/connection-pool-rds-proxy.excalidraw" alt="RDS Proxy multiplexes many app connections into few database connections" />
