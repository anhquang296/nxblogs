...
<ExcalidrawDiagram
  ...
  alt="Cluster, Spread, and Partition strategies arranging EC2 instances across hardware"
/>
...
Simply put, a Placement Group is a logical configuration that allows you to "hint" to AWS about how to arrange your EC2 Instances on physical hardware.
...
- Place them **as close together as possible** (to optimize throughput and latency).
- Or place them **as far apart as possible** (to optimize availability and hardware risk).
...
- **Use Case:** Large-scale distributed data systems such as **Hadoop HDFS, Cassandra, Kafka**. (For example: Kafka can replicate data to a partition on a different rack to ensure safety).
...
2. **Scope:** Placement Groups **do not support Multi-Region**. You cannot group instances located in Singapore and Virginia into a single PG.
