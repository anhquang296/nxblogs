...
date: '2026-05-14'
tags: ['Javascript']
description: 'Deep comparison of Browser and Node.js runtimes — JS engine, event loop implementations, available APIs, and why the same code behaves differently.'
enableComment: true
---
...
<ExcalidrawDiagram
  ...
  alt="Browser and Node.js runtime architectures placed side by side for comparison"
/>
...
Regardless of the engine — <Tooltip content="Google's Javascript engine, written in C++, powering both Chrome and Node.js">V8</Tooltip>, SpiderMonkey, or JavaScriptCore — they all go through the same pipeline:

<ExcalidrawDiagram src="/browser-vs-nodejs-engine-pipeline.excalidraw" alt="Javascript engine compilation pipeline from source code to machine code" />

1. **Lexer/Tokenizer**: Breaks source code into tokens (keywords, variables, operators)
2. **Parser**: Builds an <Tooltip content="Abstract Syntax Tree — a tree data structure describing the program's logic">AST</Tooltip>
3. **Bytecode Interpreter**: Executes bytecode immediately — fast startup, but not optimized
4. **JIT Compiler**: When it detects "hot" code (executed many times), <Tooltip content="Just-In-Time — compiles code at runtime rather than ahead of time">JIT</Tooltip> compiles it directly into highly optimized machine code
...
The answer: the <Tooltip content="A loop that coordinates async tasks on a single thread — pull a task from a queue, execute, repeat">event loop</Tooltip> — a continuous loop that checks: "is there work to do?" → execute → check again.
...
<ExcalidrawDiagram src="/browser-event-loop.excalidraw" alt="Browser event loop architecture through macrotask, microtask and render" />
...
<a href="/tools/browser-event-loop-visualizer" target="_blank" className="inline-flex items-center gap-2 rounded-lg border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-sm font-medium text-blue-400 no-underline transition hover:bg-blue-500/20">▶ Open Browser Event Loop Visualizer (interactive)</a>

### 2.2 Node.js Event Loop (libuv)

<ExcalidrawDiagram src="/nodejs-event-loop.excalidraw" alt="Node.js event loop with the six libuv phases running sequentially" />

Node.js uses a C library called <Tooltip content="Cross-platform C library providing async I/O, event loop, and thread pool — the runtime backbone of Node.js">libuv</Tooltip> to manage the event loop. Instead of a simple loop, libuv divides it into **6 phases** running sequentially:
...
<a href="/tools/nodejs-event-loop-visualizer" target="_blank" className="inline-flex items-center gap-2 rounded-lg border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-sm font-medium text-blue-400 no-underline transition hover:bg-blue-500/20">▶ Open Event Loop Architecture Visualizer (interactive)</a>
...
<Tooltip content="Web-interoperable Runtimes Community Group">WinterCG</Tooltip> is the effort to standardize a minimal shared API set across all runtimes: Node.js, Deno, Bun, Cloudflare Workers. The goal: write once, run everywhere — at least for the parts unrelated to DOM or file system.
