...
description: 'How DNS resolves domain names — recursive resolvers, root/TLD/authoritative servers, caching layers, and why the entire process completes in milliseconds.'
enableComment: true
---
...
![Journey of a DNS query through resolver, root, TLD, and authoritative server](/how-dns-work-under-the-hood.svg)
...
<Tooltip content="Domain Name System">DNS</Tooltip> is a distributed system responsible for **translating domain names into IP addresses** that computers can understand. It works like an Internet phone book: the domain name is the "person's name", the IP is the "phone number".
...
OS cache miss, the stub resolver sends the query out to a <Tooltip content="A DNS server that answers on a client's behalf: it takes one question, walks through root/TLD/authoritative servers itself, and returns the final IP.">**recursive resolver**</Tooltip> — this is the central character of the DNS system.
...
<Tooltip content="Top-Level Domain">TLD</Tooltip> is the tail part of a domain. TLD servers manage the list of all domains under a specific extension — and specifically know **where each domain's <Tooltip content="The DNS server that holds the official zone file for a domain — the ultimate source of truth for that domain's records.">authoritative server</Tooltip>** is located.

Some examples of TLD operators:

- **Verisign** — manages `.com` and `.net` (the largest in the world, nearly 200 million domains).
- <Tooltip content="Public Interest Registry">PIR</Tooltip> — manages `.org`.
- **VNNIC** — manages `.vn`.
...
<Tooltip content="Time-To-Live">TTL</Tooltip> is the number of seconds a resolver is allowed to keep a result in its cache before it must re-query. TTL is declared by the authoritative server, attached to each record.
...
Mechanism: the authoritative reads the resolver's source IP (or <Tooltip content="An extension that lets the resolver include the client's subnet in the query, so the authoritative can pick an IP close to the actual client rather than the resolver.">EDNS Client Subnet</Tooltip>) → looks up a GeoIP database → selects the appropriate IP.
