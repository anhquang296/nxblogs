...
- **Bidirectional and multi-point:** moves data between on-premises and AWS, and between regions/storage services within AWS. On the on-premises side, you install a DataSync agent (a virtual machine) to read data over <Tooltip content="Network File System — a popular file-sharing protocol on Linux/Unix">NFS</Tooltip> or <Tooltip content="Server Message Block — the Windows file-sharing protocol">SMB</Tooltip>.
...
The big picture: the on-premises agent reads data over NFS/SMB, encrypts it, and ships it over <Tooltip content="Transport Layer Security — the protocol that encrypts data in transit so it cannot be read while crossing the internet">TLS</Tooltip> to the DataSync service inside the Region, which writes it down to the storage targets — S3 (any storage class), EFS, or FSx:

<ExcalidrawDiagram
  src="/aws-datasync-architecture.excalidraw"
  alt="DataSync agent on-premises reads NFS/SMB, transfers over TLS, then writes to S3, EFS, FSx"
/>
...
<ExcalidrawDiagram
  ...
  alt="Four Storage Gateway types connect on-premises apps via NFS/SMB/iSCSI/VTL to S3, EBS snapshots, and Glacier"
/>
...
**Recovery points — the anchor for recovering after a failure.** A Volume Gateway periodically creates a <Tooltip content="Recovery point — a consistent point-in-time snapshot of a volume, stored durably in AWS; it can be used to recreate a volume or be exported as an EBS snapshot">recovery point</Tooltip> for each volume — a consistent point-in-time snapshot of the volume, stored durably in AWS. This is what disaster recovery rests on: even if the on-premises gateway hardware fails completely, the data and its recovery points remain intact in AWS.

From that, the correct recovery process when the hardware running the gateway dies completely is:

1. Deploy a new gateway appliance on-premises (to replace the failed one).
2. On the new gateway, create new volumes by cloning them from the last recovery point of the original volumes.
