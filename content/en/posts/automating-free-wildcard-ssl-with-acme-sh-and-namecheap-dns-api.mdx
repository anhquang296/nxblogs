...
date: '2025-05-02'
tags: ['Devops']
description: 'Automate free wildcard SSL certificate issuance, installation, and renewal using acme.sh with Namecheap DNS API validation.'
enableComment: true
---
...
To use the <Tooltip content="Domain validation method that uses a TXT record on DNS — required for wildcard certificates">DNS-01 Challenge</Tooltip> method (required for Wildcard SSL), you need:
...
Run the following command to start the validation and issuance process. Here I'm using Let's Encrypt as the <Tooltip content="Certificate Authority — the trusted organization that issues and vouches for SSL/TLS certificates">CA</Tooltip>.
