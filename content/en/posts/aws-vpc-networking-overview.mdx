...
When you have several sites (branches) that want to connect to each other, **VPN CloudHub** is a configuration pattern (not a separate service) that reuses the same Virtual Private Gateway as a hub: one Virtual Private Gateway + multiple Customer Gateways. The result is that the sites not only reach the VPC but can also **talk to each other** through the Virtual Private Gateway, in a hub-and-spoke topology (one center radiating to many spokes).

Because VPN CloudHub is simply multiple Site-to-Site VPN connections terminating on the same Virtual Private Gateway, it inherits the characteristics of Site-to-Site VPN: data is encrypted, travels over the public internet, and operation relies on BGP and its re-advertisement mechanism.

Once the Site-to-Site VPN connections are established, BGP lets each on-premises site advertise its address range to the central VGW. The VGW then re-advertises what it has learned back to the Customer Gateways. That is how the on-premises sites can communicate with each other through the VGW.
...
> Use DX for steady large-volume data transfer, hybrid apps that need stable latency, or reaching AWS public services while avoiding the internet entirely.
...
VPC Peering works **cross-region** and **cross-account**; and you can reference the other VPC's Security Group directly in your rules (handy for declaring permissions) — this works across accounts but only within the same region.

> Use VPC Peering only when you need to connect a small number of VPCs over a simple, private path. When the VPC count grows large, the VPC peering connections between them form a tangled mesh that becomes unmanageable — and that's exactly when Transit Gateway enters.
...
> A Gateway Endpoint **cannot** be reached from on-prem (over DX/VPN). If the question says "need to access S3 privately from the on-prem data center," the answer is an **Interface Endpoint** for S3, not a Gateway Endpoint.
