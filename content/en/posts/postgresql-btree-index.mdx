---
title: 'PostgreSQL Index: Why Indexes Speed Up Queries, And When Postgres Ignores Them'
date: '2026-05-29'
tags: ['Database']
description: 'A dissection of the B-tree index in PostgreSQL — from the tree structure, leaf nodes and ctid, the logarithmic power of tree descent, the page-as-unit read model, to how the planner weighs random I/O when deciding whether to use an index at all.'
enableComment: true
---

# PostgreSQL Index: Why Indexes Speed Up Queries, And When Postgres Ignores Them
...
An index designed around the table's structure (e.g. "just index every foreign key") is usually wasteful; an index designed around the **actual query pattern** is what makes the difference.
...
## 7. Indexes bloat too — a little on maintenance
...
- **Random I/O is the Achilles' heel.** When matching rows are scattered and numerous enough, the planner abandons the index and picks **Seq Scan** — one sequential pass over the table beats millions of random `ctid` fetches.
- **Design indexes around how the application queries**, prioritising **high-cardinality** columns — the ones that show up in `WHERE`, `JOIN`, and `ORDER BY`.
