...
<ExcalidrawDiagram src="/rate-limiting-fixed-window.excalidraw" alt="Fixed window counter counts requests per fixed window then resets to 0" />
...
<ExcalidrawDiagram src="/rate-limiting-token-bucket.excalidraw" alt="Token bucket refills tokens steadily, each request spends one token, reject when empty" />
...
> **Note:** the code above **has a race condition** — read and write are not atomic. Production must wrap it in a <Tooltip content="A Lua program executed atomically inside Redis via the EVAL command, letting multiple reads and writes run as a single indivisible step.">Lua script</Tooltip> (see section 4).
...
<ExcalidrawDiagram src="/rate-limiting-leaky-bucket.excalidraw" alt="Leaky bucket queues requests FIFO, draining at a fixed rate to smooth the output" />
