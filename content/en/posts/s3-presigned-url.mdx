...
<ExcalidrawDiagram src="/s3-presigned-url-architecture.excalidraw" alt="Backend only signs URLs while the client uploads and downloads directly with S3" />
...
<ExcalidrawDiagram src="/s3-presigned-url-upload-flow.excalidraw" alt="File upload flow via presigned POST from client to S3" />
...
<ExcalidrawDiagram src="/s3-presigned-url-download-flow.excalidraw" alt="Private document download flow via presigned GET URL" />
...
<ExcalidrawDiagram src="/s3-presigned-url-cloudfront-cookies.excalidraw" alt="Issuing CloudFront signed cookies to display images via the CDN" />
