...
<ExcalidrawDiagram src="/sorted-set-overview.excalidraw" alt="Sorted Set stores member-score pairs, auto-sorted by ascending score" />
...
<ExcalidrawDiagram src="/sorted-set-hash-table.excalidraw" alt="Hash table maps member to score for O(1) lookup" />
...
![Multi-level skip list, top levels skip many nodes like express trains](/sorted-set-skiplist.svg)
...
![Finding score 2100 on a skip list, top to bottom in 5 steps](/sorted-set-skiplist-search.svg)
...
![Span field records nodes skipped, accumulated to compute ZRANK in O(log N)](/sorted-set-span.svg)
...
<ExcalidrawDiagram src="/sorted-set-listpack.excalidraw" alt="Listpack stores adjacent member-score pairs in a contiguous memory block" />
...
<ExcalidrawDiagram src="/sorted-set-encoding-decision.excalidraw" alt="Flowchart choosing listpack or skiplist+hashtable encoding by two thresholds" />
...
<ExcalidrawDiagram src="/sorted-set-dual-structure.excalidraw" alt="Hash table and skip list cooperate, each command routed to the right structure" />
