...
<ExcalidrawDiagram
  ...
  alt="Two ways a user in Account A can write to an S3 bucket in Account B: Method 1 — assume a cross-account IAM Role in Account B (giving up its own permissions) then write to S3; Method 2 — write directly, authorized by the S3 bucket policy (keeping its own permissions)"
/>
...
<ExcalidrawDiagram
  ...
  alt="IAM policy evaluation logic flowchart: the decision starts at Deny, then passes through six stages in order — explicit Deny, Organizations SCPs, resource-based policies, identity-based policies, permissions boundaries, and session policies; each stage must allow, an explicit Deny rejects immediately, ending in a final Allow or Deny"
/>
...
<ExcalidrawDiagram
  ...
  alt="IAM Identity Center diagram: inside an AWS Organization there is a management account, an OU (Development) with Dev Accounts A and B, and an OU (Production) with Prod Accounts A and B; IAM Identity Center lives in the management account and holds a Developers group (Bob, Alice) plus ReadOnlyAccess and FullAccess permission sets; the Developers group is assigned the FullAccess permission set, which provisions a full-access IAM role into Dev Account B"
/>
