...
### 2.1. An EC2 Instance Acting as NAT
...
AWS used to provide a pre-configured Amazon Linux AMI for NAT Instances, but this AMI reached the end of standard support on December 31, 2020. If you want to use a NAT Instance today, you have 2 options: find an alternative AMI on the AWS Marketplace, or configure it yourself from a standard Linux AMI — enable IP forwarding in the kernel (`net.ipv4.ip_forward = 1`) and set up iptables NAT rules.
...
Since a NAT Instance is an EC2, you can (and must) attach a Security Group. This is both an advantage and a responsibility.
...
**Note**: EC2 instances in the same subnet as the NAT Gateway cannot use it — only EC2 instances from other subnets can route traffic through the NAT Gateway. This isn't a technical limitation of the NAT Gateway; it's a consequence of how route tables work.

A route table is attached to a subnet, and within one route table each destination CIDR (destination IP range) can have exactly one route — you can't have two `0.0.0.0/0` entries at once. The NAT Gateway lives in the public subnet, so that subnet's route table must have `0.0.0.0/0 → igw-xxx` for the NAT Gateway itself to reach the internet. To make an EC2 in that same subnet go through the NAT Gateway, you'd have to change that entry to `0.0.0.0/0 → nat-xxx` — but doing so overwrites the route to the IGW, leaving even the NAT Gateway with no path to the internet, and traffic loops back to itself. The two routes overlap and are mutually exclusive.

The fix is therefore to split subnets: the NAT Gateway goes in a public subnet (route table pointing `0.0.0.0/0 → igw-xxx`), while EC2 instances that need NAT sit in a private subnet with their own route table pointing `0.0.0.0/0 → nat-xxx`. Each subnet has an independent route table, so the two routes no longer conflict.
