...
A colleague suggests: _"Why not use a NAT Instance for dev? Run it on a `t3.micro` — costs just a few bucks."_
...
By default, AWS enables **Source/Destination Check** on every EC2 instance. This feature checks: _does the packet arriving at or leaving this instance have a source or destination IP that matches the instance's own IP?_ If not — the packet is **dropped** immediately.
