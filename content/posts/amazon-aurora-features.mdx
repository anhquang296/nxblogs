...
**Copy-on-write** là kỹ thuật chia sẻ data giữa hai cluster — chỉ thực sự copy data **khi có thay đổi**. Đây là điểm mấu chốt giúp clone vừa nhanh vừa rẻ.
...
Clone **không copy gì cả** — nó chỉ "trỏ chung" đến data của production.

**Bước 2: Đọc trên clone** → đọc trực tiếp từ storage gốc, không phát sinh copy.

**Bước 3: Khi clone ghi thay đổi**

```
Clone: UPDATE users SET name='Bob' WHERE id=5
   ↓
Aurora phát hiện page chứa id=5 sắp bị thay đổi
   ↓
Copy riêng page đó cho clone (chỉ page này)
   ↓
Clone ghi vào bản copy → production giữ nguyên page gốc
```
