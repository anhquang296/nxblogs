...
<ExcalidrawDiagram src="/rate-limiting-fixed-window.excalidraw" alt="Fixed window counter đếm request mỗi cửa sổ cố định rồi reset về 0" />
...
<ExcalidrawDiagram src="/rate-limiting-token-bucket.excalidraw" alt="Token bucket nạp token đều đặn, mỗi request tiêu một token, hết token thì reject" />
...
> **Lưu ý:** code trên **có race condition** — đọc và ghi không atomic. Production phải bọc bằng <Tooltip content="Đoạn mã Lua chạy atomic bên trong Redis qua lệnh EVAL, cho phép nhiều thao tác đọc-ghi xảy ra như một bước duy nhất.">Lua script</Tooltip> (xem mục 4).
...
<ExcalidrawDiagram src="/rate-limiting-leaky-bucket.excalidraw" alt="Leaky bucket xếp request vào hàng đợi FIFO, rút ra với tốc độ cố định để làm mượt output" />
