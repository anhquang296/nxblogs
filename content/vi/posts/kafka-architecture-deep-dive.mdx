...
Tên file chính là **base offset** — offset của message đầu tiên trong segment đó. Khi segment đạt giới hạn kích thước (mặc định 1GB) hoặc thời gian, Kafka tạo segment mới:
...
Khi consumer chết và restart (hoặc khi có rebalance trong group, nghĩa là có 1 consumer mới join vào consumer group), Kafka tra cứu `__consumer_offsets` để biết offset cuối cùng đã commit, rồi bắt đầu đọc từ vị trí tiếp theo. Consumer không cần nhớ gì — Kafka nhớ hộ.
...
Disk → Kernel Buffer (copy 1) → NIC Buffer (copy 2)
              ↓
    Dữ liệu KHÔNG BAO GIỜ đi qua application
```
