...
Khi consumer chết và restart (hoặc khi có rebalance trong group), Kafka tra cứu `__consumer_offsets` để biết offset cuối cùng đã commit, rồi bắt đầu đọc từ vị trí tiếp theo. Consumer không cần nhớ gì — Kafka nhớ hộ.
