...
description: 'Hướng dẫn deploy Shopify App — hiểu rõ sự tách biệt giữa deploy vật lý (cloud hosting) và đăng ký logical app trên Shopify.'
enableComment: true
---
...
- <Tooltip content="Công cụ dòng lệnh chính thức của Shopify để tạo, phát triển và deploy app">Shopify CLI</Tooltip> — đã cài đặt `@shopify/cli`
...
- <Tooltip content="các quyền cơ bản như `read_products`, `write_products`, `read_orders`. Bạn có thể sử dụng ngay mà không cần phê duyệt">Standard scopes</Tooltip>
- <Tooltip content="các quyền liên quan đến dữ liệu nhạy cảm hoặc hành động ảnh hưởng lớn (tạo đơn hàng, truy cập toàn bộ customer data, ...). Shopify yêu cầu bạn phải giải thích lý do và được phê duyệt trước khi sử dụng">Restricted scopes</Tooltip>
...
- <Tooltip content="hay còn gọi là API Key">Client ID</Tooltip>
- <Tooltip content="hay còn gọi là API Secret Key">Client Secret</Tooltip>
...
- <Tooltip content="HTTP callback do Shopify gửi tới URL của bạn khi có sự kiện xảy ra (đơn hàng mới, sản phẩm cập nhật, ...)">Webhooks</Tooltip> được nhận (nếu app có đăng ký) — kiểm tra logs trên cloud
