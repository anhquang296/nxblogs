...
| Lựa chọn | Bản chất | Kết nối on-premises | Khi nào dùng |
| --- | --- | --- | --- |
| **AWS Managed Microsoft AD** | AD Microsoft thật, do AWS vận hành | Có — lập trust hai chiều với AD on-premises | Cần đầy đủ tính năng AD, MFA, chạy workload Windows/AD-aware trên AWS, đồng thời liên thông với AD công ty |
| **AD Connector** | Cổng proxy, chuyển tiếp yêu cầu xác thực về AD on-premises | Bắt buộc — chỉ là cầu nối | Muốn dùng dịch vụ AWS bằng tài khoản AD on-premises có sẵn, không lưu gì trên cloud |
...
- **Managed Microsoft AD** là lựa chọn duy nhất là AD Microsoft thực thụ và có thể lập trust với AD on-premises (người dùng on-premises đăng nhập tài nguyên AWS mà không cần nhân bản tài khoản).
- **AD Connector** không lưu trữ bất kỳ dữ liệu danh tính nào trên AWS — mọi yêu cầu xác thực đều được chuyển thẳng về AD on-premises. Nếu đề nói "không muốn để dữ liệu danh tính trên cloud, chỉ proxy về AD nội bộ" → đây là đáp án.
...
> **Bẫy thường gặp:** "Chỉ chuyển tiếp xác thực về AD on-premises, không giữ dữ liệu trên AWS" → **AD Connector**. "Cần AD Microsoft thật + trust với on-premises" → **Managed Microsoft AD**. "Nhỏ, rẻ, độc lập, không cần trust" → **Simple AD**.
...
| **Proxy** xác thực về AD on-premises, không lưu dữ liệu trên cloud | AD Connector |
| **AD Microsoft thật** trên AWS + trust với on-premises, hỗ trợ MFA | AWS Managed Microsoft AD |
