...
description: 'Bắt đầu với AWS — hiểu global infrastructure (Regions, AZs), IAM users/groups/roles/policies, và best practices bảo mật tài khoản AWS.'
enableComment: true
---

# Hành trình "nhập môn" AWS: Từ hạ tầng toàn cầu đến tư duy bảo mật với IAM

Chào mọi người, mình đang trong quá trình "cày cuốc" AWS và nhận ra rằng, trước khi chạm tay vào những dịch vụ phức tạp, việc hiểu rõ **Hạ tầng (Infrastructure)** và <Tooltip content="Identity and Access Management — dịch vụ của AWS dùng để quản lý người dùng, nhóm, vai trò và quyền truy cập vào các tài nguyên trong tài khoản">Quản lý định danh (IAM)</Tooltip> là bước đệm quan trọng nhất.
...
Nếu bạn từng nghe về <Tooltip content="Content Delivery Network — mạng lưới máy chủ phân tán toàn cầu giúp đưa nội dung đến gần người dùng cuối nhất, giảm độ trễ truy cập">CDN</Tooltip>, thì đây chính là nó. Với hơn 400 điểm tại 90 thành phố, Edge Location giúp đưa nội dung đến gần người dùng nhất có thể, giảm thiểu độ trễ (latency).
...
Khi vừa tạo tài khoản, bạn có **Root Account**. Lời khuyên xương máu là: **Đừng bao giờ dùng Root Account cho công việc hằng ngày.** Hãy dùng nó để tạo một User admin riêng cho mình, thiết lập <Tooltip content="Multi-Factor Authentication — cơ chế xác thực yêu cầu nhiều yếu tố (ví dụ mật khẩu + mã OTP từ điện thoại) để tăng cường bảo mật">MFA</Tooltip> và cất "chiếc chìa khóa" Root này vào tủ kính.
