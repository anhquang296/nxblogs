...
description: '6 bẫy cache kinh điển — Cache Avalanche, Stampede, Penetration, Breakdown — mà ngay cả senior cũng mắc phải, và cách phòng tránh.'
enableComment: true
---
...
**Định nghĩa:** <Tooltip content="Hàng loạt key hết hạn cùng lúc gây quá tải DB">Cache Avalanche</Tooltip> xảy ra khi một lượng lớn key trong Cache **hết hạn hoặc bị xóa cùng một lúc**. Hàng loạt request bị Cache Miss và ồ ạt đổ xuống Database, khiến DB bị quá tải.
...
**Định nghĩa:** Tương tự Cache Avalanche, nhưng chỉ xảy ra với **một <Tooltip content="Key có lượng truy cập cực cao tại một thời điểm">Hot Key</Tooltip>** (ví dụ "Kết quả xổ số lúc 18h30"). Khi Hot Key hết hạn, một lượng lớn request ồ ạt đổ xuống DB.
...
**Định nghĩa:** <Tooltip content="Truy vấn liên tục dữ liệu không tồn tại để bypass Cache">Cache Penetration</Tooltip> xảy ra khi user (hoặc hacker) liên tục query dữ liệu **không có trong Cache và cũng không có trong Database**. Ví dụ: hacker gửi hàng triệu request tìm `user_id = -1` hoặc UUID ngẫu nhiên → DB bị DDoS.
...
<Tooltip content="Probabilistic Data Structure">Bloom Filter</Tooltip> là cấu trúc dữ liệu xác suất cực kỳ hiệu quả về bộ nhớ, trả lời theo kiểu:

- <Tooltip content="Definitely No">"Chắc chắn không có"</Tooltip>
- <Tooltip content="Possibly Yes — False Positive">"Có thể có"</Tooltip>
