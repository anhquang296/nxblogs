...
Để ép buộc chỉ dùng HTTPS, ta thêm vào bucket policy một điều kiện từ chối khi `aws:SecureTransport` bằng `false`.

Như đã nói, SSE-C bắt buộc HTTPS sẵn.
...
Ví dụ policy dưới đây từ chối mọi `s3:PutObject` không mã hóa bằng SSE-KMS. Điều kiện dựa trên `s3:x-amz-server-side-encryption` — header mà client khai báo kiểu mã hóa khi upload:
...
Bucket policy được kiểm tra trước khi Default Encryption áp dụng. Nghĩa là policy là "người gác cổng" — nó có thể chặn đứng một request không hợp lệ ngay từ đầu, còn Default Encryption chỉ là lưới an toàn áp mã hóa cho những request đã được cho qua.
...
Trong khi Object Lock bảo vệ object trong S3, **Glacier Vault Lock** bảo vệ dữ liệu trong kho lưu trữ lạnh Amazon S3 Glacier. Nó cũng theo mô hình WORM, nhưng điểm đặc trưng nằm ở **lock policy** -- một khi đã khóa, chính sách đó trở thành bất biến.
