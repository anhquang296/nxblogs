...
Mỗi object được mã hóa bằng một khóa riêng, không dùng chung một khóa cho cả bucket. Bản thân khóa đó lại được mã hóa thêm một lần nữa bằng một root key do AWS quản lý và xoay vòng định kỳ. Cơ chế lồng khóa này gọi là **envelope encryption** — dùng một khóa để mã hóa dữ liệu, rồi dùng một khóa khác để mã hóa chính khóa đó, giúp việc xoay vòng root key không phải mã hóa lại toàn bộ dữ liệu.
...
Để giải quyết vấn đề throttling khi sử dụng SSE-KMS, **S3 Bucket Keys** ra đời — S3 tạo một khóa cho bucket (gọi là bucket key), bucket sẽ dùng bucket key để sinh data key cho từng object thay vì gọi KMS mỗi lần, giảm số lời gọi KMS tới khoảng 99% (giảm cả chi phí lẫn nguy cơ throttling).
...
Khi bật Bucket Keys, S3 gọi KMS một lần để sinh ra bucket key; KMS trả về bucket key ở hai dạng cùng lúc: bản rõ để S3 dùng ngay, và bản đã được mã hóa bằng KMS key để lưu lại. S3 giữ bản rõ trong bộ nhớ đệm một khoảng thời gian ngắn, và trong khung thời gian đó, khóa dữ liệu cho từng object được sinh và mã hóa cục bộ bằng bucket key, không cần đụng tới KMS nữa. Chuỗi bọc khóa vì thế là: KMS key bọc bucket key, bucket key bọc data key, và data key mới thực sự mã hóa object.

Trong metadata của mỗi object, S3 lưu cả hai lớp: data key đã mã hóa bằng bucket key, và bucket key đã mã hóa bằng KMS key. Khi đọc object, S3 chỉ cần một lượt gọi `Decrypt` lên KMS để mở bucket key rồi cache lại, sau đó tự mở data key và giải mã object hoàn toàn cục bộ. Các object khác dùng chung bucket key đó không phải gọi KMS lần nào — đây chính là lý do số lời gọi KMS giảm mạnh.

Ba lý do khiến bucket key không phải là khóa gốc:

- **Quyền kiểm soát vẫn nằm trọn ở KMS key.** Nếu bạn disable KMS key hoặc thu hồi quyền trên nó, bucket key không còn giải mã được và toàn bộ dữ liệu trở nên không truy cập được. Mọi key policy, grant và dấu vết audit đều xoay quanh KMS key, không phải bucket key.
- **Vòng đời rất ngắn.** S3 xoay bucket key liên tục; một bucket không chỉ có duy nhất một bucket key mà nhiều khóa được sinh ra theo thời gian và theo ngữ cảnh request. Nó giống một khóa cache tạm thời hơn là một khóa định danh lâu dài.
- **Phạm vi bị bó hẹp.** Mỗi bucket key chỉ gắn với đúng một bucket và một KMS key nguồn, không thể dùng chéo sang bucket khác.

Đánh đổi ở đây là bán kính ảnh hưởng (blast radius — phạm vi thiệt hại nếu một khóa bị lộ) của bucket key rộng hơn data key một chút: nếu một bucket key bị lộ, các object nằm trong khung thời gian nó phục vụ đều bị ảnh hưởng, thay vì chỉ một object. AWS chấp nhận đánh đổi này vì bucket key sống ngắn và chỉ tồn tại ở dạng bản rõ trong bộ nhớ của S3; xét về mô hình đe dọa, nếu kẻ tấn công lấy được khóa bản rõ từ bên trong S3 thì mô hình hai cấp cũ cũng đã thất thủ rồi.
