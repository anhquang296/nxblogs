...
Hệ thống của bạn đã lớn lên. Ban đầu chỉ có một <Tooltip content="Virtual Private Cloud — mạng riêng ảo của bạn trong AWS, nơi đặt các tài nguyên như EC2, RDS">VPC</Tooltip> gọn gàng. Giờ thì có hàng chục VPC nằm rải rác trên nhiều account, cộng thêm một data center on-premises (tại chỗ, do công ty tự vận hành) vẫn còn giữ phần lớn dữ liệu nội bộ. Và một loạt câu hỏi kết nối bắt đầu dồn tới:
...
Bài viết này là tấm bản đồ giúp bạn phân biệt chúng. Với mỗi dịch vụ, mình sẽ đi qua: nó giải quyết vấn đề gì, tính năng cốt lõi, use case thực tế, và những điểm hay bị hỏi trong phòng thi <Tooltip content="AWS Certified Solutions Architect – Associate, chứng chỉ kiến trúc sư giải pháp AWS mức Associate">SAA</Tooltip>.
...
Một ý xuyên suốt cần ghim trước: khi nối hai mạng riêng (VPC với VPC, hay VPC với on-premises), nguyên tắc nền tảng là các dải địa chỉ phải **không trùng nhau**. Mỗi mạng được mô tả bằng một <Tooltip content="Classless Inter-Domain Routing — cách viết một dải địa chỉ IP, ví dụ 10.0.0.0/16 gồm các IP từ 10.0.0.0 đến 10.0.255.255">CIDR</Tooltip> (một dải IP). Nếu hai mạng dùng chung dải IP, router không biết một địa chỉ thuộc về bên nào — nên mọi hình thức kết nối ngang hàng đều yêu cầu CIDR không đè lên nhau. Giữ ý này trong đầu, giờ ta đi qua từng nhóm.
...
Nền tảng của nó là <Tooltip content="Internet Protocol Security — bộ giao thức tạo đường hầm mã hóa giữa hai mạng, bảo vệ dữ liệu khi truyền qua mạng công cộng">IPsec</Tooltip> — bộ giao thức bọc gói tin trong một "đường hầm" mã hóa, để dù dữ liệu đi qua internet thì kẻ nghe lén ở giữa cũng chỉ thấy mớ byte vô nghĩa. Một kết nối VPN cần hai đầu, mỗi đầu là một "cổng":

- **Virtual Private Gateway** là cổng VPN phía AWS — bạn gắn nó vào VPC. Có thể đặt một <Tooltip content="Autonomous System Number — số định danh một mạng tự trị trong giao thức định tuyến BGP; mỗi bên của kết nối BGP có một Autonomous System Number riêng">Autonomous System Number</Tooltip> tùy chỉnh cho Virtual Private Gateway nếu muốn.
...
1. **Server on-prem tạo request.** Server gửi gói tin đến đích `10.0.1.20`. Nó không biết gì về VPN — chỉ thấy đây là một địa chỉ thuộc mạng khác, nên đẩy gói tin tới <Tooltip content="Default gateway — router mặc định mà một thiết bị gửi gói tin tới khi đích không nằm trong mạng nội bộ của nó">default gateway</Tooltip> (router nội bộ).
...
<ExcalidrawDiagram
  ...
  alt="Luồng đi của một request qua Site-to-Site VPN: từ server on-premises (192.168.1.10) tạo request, qua Customer Gateway mã hóa IPsec, đi qua đường hầm trên internet công cộng, tới Virtual Private Gateway giải mã, được route table của VPC định tuyến vào subnet, và Security Group (cho phép dải 192.168.1.0/24) chuyển tới EC2 (10.0.1.20); một đường phản hồi nét đứt đi ngược lại cho thấy traffic hai chiều đều nằm trong tunnel, không cần Internet Gateway hay IP public"
/>
...
Trên một kết nối DX, bạn tạo các <Tooltip content="Virtual Interface — giao diện ảo cấu hình trên kết nối Direct Connect để định tuyến lưu lượng tới một loại đích cụ thể">VIF</Tooltip> (Virtual Interface) tùy theo bạn muốn tới đâu:
...
<ExcalidrawDiagram
  ...
  alt="VPC Peering không bắc cầu: VPC A peering với VPC B và VPC B peering với VPC C, cả hai kết nối đều hoạt động, nhưng đường đi từ A tới C bị chặn vì peering không có tính bắc cầu; muốn A kết nối tới C phải tạo một peering A–C trực tiếp riêng"
/>
...
<ExcalidrawDiagram
  ...
  alt="Transit Gateway nối nhiều VPC xuyên account theo mô hình hub-and-spoke: bên trong AWS Cloud có 'your account' chứa VPC 1 và VPC 2, cùng 'third-party account' chứa VPC 3 và VPC 4, mỗi VPC đều có các instance; cả bốn VPC đều gắn vào một Transit Gateway ở trung tâm, và một route table gắn với Transit Gateway điều khiển định tuyến giữa các VPC"
/>
...
- **Cross-account qua RAM:** chia sẻ TGW cho các account khác bằng <Tooltip content="AWS Resource Access Manager — dịch vụ chia sẻ tài nguyên AWS giữa nhiều account trong một tổ chức">RAM</Tooltip>.
...
- **Hỗ trợ IP multicast:** TGW là dịch vụ AWS **duy nhất** hỗ trợ <Tooltip content="IP multicast — gửi một gói tin tới đồng thời nhiều người nhận đã đăng ký, thay vì gửi riêng từng bản sao tới từng người">IP multicast</Tooltip>. Thấy "multicast" thì nghĩ ngay TGW.
- **Tăng băng thông VPN bằng ECMP:** một Site-to-Site VPN bị giới hạn ~1.25 Gbps mỗi tunnel. Với TGW, bạn dùng <Tooltip content="Equal-Cost Multi-Path routing — định tuyến chia tải đồng thời qua nhiều đường có cùng 'chi phí', nhờ đó cộng dồn băng thông của các đường lại">ECMP</Tooltip> để gộp nhiều kết nối VPN song song lại, cộng dồn băng thông vượt qua trần của một kết nối đơn.
...
- Hoạt động bằng cách dựng một <Tooltip content="Elastic Network Interface — card mạng ảo gắn vào instance/subnet, mang một IP private trong VPC">ENI</Tooltip> mang IP private ngay trong subnet của bạn. Vì là ENI nên nó dùng **Security Group** để kiểm soát truy cập.
...
- **Use case:** đưa lưu lượng vào các thiết bị <Tooltip content="Intrusion Detection System — hệ thống phát hiện xâm nhập, phân tích lưu lượng mạng để nhận diện hành vi tấn công">IDS</Tooltip>, soi nội dung (content inspection), gỡ lỗi mạng, điều tra sau sự cố.

### 9.2. AWS Network Firewall

Để hiểu vì sao cần dịch vụ này, hãy nhớ giới hạn của những công cụ lọc sẵn có. Security Group và <Tooltip content="Network Access Control List — danh sách quy tắc lọc lưu lượng ở mức subnet, hoạt động không trạng thái (stateless)">NACL</Tooltip> chỉ làm việc ở tầng 3/4 (IP và cổng). <Tooltip content="Web Application Firewall — tường lửa lọc lưu lượng HTTP/HTTPS tầng 7, chặn web exploit như SQL injection, XSS">WAF</Tooltip> lọc HTTP ở tầng 7 nhưng chỉ gắn được lên ALB, CloudFront, API Gateway. Vậy nếu cần bảo vệ **toàn bộ một VPC** ở tầng mạng, với khả năng soi gói tin sâu, thì sao?
...
- Rule đa dạng: stateful/stateless, lọc theo **tên miền** (cho/cấm danh sách domain, URL), ngăn chặn xâm nhập (<Tooltip content="Intrusion Prevention System — hệ thống ngăn chặn xâm nhập, không chỉ phát hiện mà còn chủ động chặn lưu lượng tấn công">IPS</Tooltip>), soi theo chữ ký.
