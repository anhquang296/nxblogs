...
date: '2026-01-26'
tags: ['Devops', 'AWS', 'SAA']
description: 'AWS EC2 Placement Groups — chiến lược Cluster, Spread, Partition cho tối ưu network performance, fault tolerance, hoặc big data workloads.'
enableComment: true
---
...
<ExcalidrawDiagram
  src="/ec2-instance-placement-groups-diagram.excalidraw"
  alt="Ba chiến lược Cluster, Spread và Partition sắp xếp EC2 instance trên phần cứng"
/>
...
Hiểu đơn giản, Placement Group là một cấu hình logic cho phép bạn "gợi ý" với AWS về cách sắp xếp các EC2 Instances trên phần cứng vật lý.
...
- Đặt chúng **gần nhau nhất có thể** (để tối ưu throughput, latency).
- Hoặc đặt chúng **xa nhau nhất có thể** (để tối ưu availability, rủi ro phần cứng).

Hiện tại, AWS cung cấp 3 chiến lược chính: **Cluster**, **Spread**, và **Partition**.
...
- **Ưu điểm:**
  - Latency cực thấp.
  - Throughput cực cao giữa các node (lên đến 10-100 Gbps).
...
- **Ưu điểm:** Availability cực cao. Việc hỏng hóc phần cứng ở instance này chắc chắn không ảnh hưởng đến instance kia.
- **Nhược điểm:** Bị giới hạn số lượng. Bạn chỉ được tối đa **7 running instances** cho mỗi AZ trong một Spread PG. Lý do đơn giản là không gian vật lý để tách biệt hoàn toàn là có hạn.

- **Use Case:** Thích hợp cho các ứng dụng quan trọng, các cụm cơ sở dữ liệu nơi mà sự sống còn của từng node là ưu tiên hàng đầu.
...
- **Use Case:** Các hệ thống phân tán dữ liệu lớn như **Hadoop HDFS, Cassandra, Kafka**. (Ví dụ: Kafka sẽ biết để replicate data sang partition khác rack nhằm đảm bảo an toàn).
...
2. **Phạm vi:** Placement Group **không hỗ trợ Multi-Region**. Bạn không thể gom nhóm các instance nằm ở Singapore và Virginia vào chung một PG.
3. **Chiến lược tối ưu:**
