...
description: 'Implement idempotent payment API — từ nền tảng toán học đến code Node.js + Redis thực tế ngăn chặn thanh toán trùng lặp.'
enableComment: true
---
...
Đây là cơn ác mộng kinh điển của mọi đội ngũ làm payment, và là lý do tồn tại của một khái niệm tưởng nghe rất hàn lâm: <Tooltip content="Tính chất của một thao tác mà thực hiện 1 lần hay N lần đều cho cùng kết quả trên hệ thống">Idempotency</Tooltip>. Bài này sẽ đi từ nguồn gốc toán học của nó, sang cách nó hoạt động trong API, và cuối cùng là một implementation chạy được cho **Create Payment** với Node.js + Redis.
...
Còn `POST /payments` thì **không** tự nhiên idempotent. Mỗi lần POST mặc định = một payment mới. Để biến nó thành idempotent, ta cần một <Tooltip content="định danh do client cung cấp, không phải server sinh ra, để gắn nhãn duy nhất cho một ý định thao tác">định danh ngoại</Tooltip> — đó chính là <Tooltip content="Một chuỗi duy nhất (thường là UUID v4) gắn vào HTTP header để server nhận biết các retry của cùng một ý định">Idempotency Key</Tooltip>.
...
2. **Đừng bắt đầu phức tạp.** Pattern `SET NX + TTL + request hash` ở trên đã bao phủ 90% use case. Lease, <Tooltip content="Giao thức điều phối phân tán gồm pha prepare và pha commit, đảm bảo tất cả node cùng commit hoặc cùng rollback">two-phase commit</Tooltip>, outbox pattern chỉ cần khi bạn ở quy mô như Stripe hay AWS.
