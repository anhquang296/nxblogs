...
date: '2025-01-12'
tags: ['Devops', 'AWS']
description: 'Quản lý environment variables an toàn cho AWS App Runner bằng Secrets Manager — tạo secret, cấu hình IAM roles, và reference trong service.'
enableComment: true
---
...
5. Chọn khóa mã hóa (Encryption key): Bạn có thể dùng khóa mặc định của AWS hoặc tạo khóa <Tooltip content="AWS Key Management Service — dịch vụ quản lý khóa mã hóa của AWS, dùng để mã hóa/giải mã dữ liệu nhạy cảm như secrets, S3 objects, EBS volumes.">KMS</Tooltip> riêng.
...
Đây là bước quan trọng nhất. Mặc định, App Runner sẽ không có quyền "mở két" để lấy thông tin. Bạn cần cấp quyền thông qua một <Tooltip content="Identity and Access Management Role — danh tính AWS có thể được gán cho dịch vụ (như App Runner) để cấp quyền truy cập các resource khác mà không cần dùng access key.">IAM Role</Tooltip>.
