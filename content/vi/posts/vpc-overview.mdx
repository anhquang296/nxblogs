...
Bạn vừa deploy một API e-commerce lên AWS. Theo best practice, EC2 được đặt trong **private subnet** — không có đường đi trực tiếp từ internet vào. Một **Application Load Balancer** (ALB — bộ cân bằng tải ứng dụng) ngồi ở **public subnet** đón request từ client rồi forward vào EC2.
...
Trước khi packet chạm bất kỳ component nào, nó cần biết mình đang đi vào đâu. **VPC** (Virtual Private Cloud — mạng riêng ảo) chính là ranh giới đó.
...
> Một chi tiết thú vị: khi [EC2](/vi/posts/learn-ec2-part-1) boot lên, hệ điều hành bên trong nhận IP thông qua **DHCP** (Dynamic Host Configuration Protocol — giao thức tự động cấp phát cấu hình mạng cho thiết bị). Thực tế IP đã được AWS pick sẵn từ **trước khi OS boot** — khi tạo ENI, control plane chọn IP từ subnet rồi lưu vào database. Khi đó, EC2 không biết đến địa chỉ này và cần sử dụng DHCP protocol để xác nhận IP đó.
...
Packet từ client `1.2.3.4` với destination `52.10.10.10` (public IP của ALB) đã đến edge network của AWS. Component đầu tiên nó gặp là **Internet Gateway** (IGW — cổng kết nối VPC với internet).
...
**NAT Gateway** (Network Address Translation Gateway — cổng dịch địa chỉ mạng) nằm ở **public subnet**, có Elastic IP, cho phép resource ở private subnet **gửi** request ra internet nhưng **chặn** internet gửi request vào.
...
1. EC2 (10.0.2.50) tạo packet `src 10.0.2.50`, `dst 3.4.5.6`
2. Route Table (private subnet) match 3.4.5.6 với rule `0.0.0.0/0 → nat-xxx`. Forward đến NAT Gateway
...
4. Route Table (public subnet) match 3.4.5.6 với rule 0.0.0.0/0 → igw-xxx. Forward đến IGW
