...
<ExcalidrawDiagram
  ...
  alt="Stack 4 tầng của sequential read — Query executor (Seq Scan vs Index Scan), PostgreSQL I/O layer (read() 8KB, multi-block read), Kernel (page cache + read-ahead), Storage (platter, seek + rotational latency); mỗi tầng tự quyết định dựa trên thông tin riêng nó có"
/>
...
<ExcalidrawDiagram
  ...
  alt="So sánh hai pattern — sequential đọc các block 100,101,102… liền kề, đầu đọc trôi liên tục đạt ~150 MB/s; random nhảy tới 10, 4732, 88, 2901 với một lần seek + rotation mỗi bước, throughput sụp xuống vài trăm KB/s"
/>
...
<ExcalidrawDiagram
  ...
  alt="Ba con số độc lập của read() — (a) ứng dụng yêu cầu count byte; (b) kernel chạm đĩa bao nhiêu là do page cache + read-ahead quyết định, không phải count; (c) thiết bị luôn làm việc theo nguyên block 4KB / sector 512B"
/>
...
<ExcalidrawDiagram
  ...
  alt="Vòng đời cửa sổ read-ahead — block 0 cache miss kích hoạt đọc đồng bộ và prefetch bất đồng bộ block 1–15; ứng dụng đọc 0,1,2,3 toàn cache hit; khi chạm readahead marker ở block 12, kernel phóng tiếp cửa sổ block 16–47 trong nền"
/>
...
<ExcalidrawDiagram
  ...
  alt="Logic vs vật lý — các page heap file 0..4 liền mạch về logic được scan theo thứ tự, nhưng ánh xạ tới các extent rải rác trên platter, khiến đầu đọc phải nhảy qua lại; một seq scan biến thành random I/O vật lý"
/>
