...
- **AWS Snowball Edge** — "con ngựa thồ" cho migration lớn, có hai biến thể:
  ...
  - **Compute Optimized**: nhiều vCPU/RAM hơn, để chạy EC2 và Lambda ngay trên thiết bị — phục vụ <Tooltip content="Edge computing — xử lý dữ liệu ngay tại nơi sinh ra dữ liệu thay vì gửi hết về cloud, hữu ích khi mạng yếu/chập chờn hoặc cần độ trễ thấp">edge computing</Tooltip> ở những nơi mạng yếu.
  - <Tooltip content="Block storage là khối ổ đĩa thô như EBS; object storage là kho object như S3 (lưu file kèm metadata, truy cập qua API)">Cả block lẫn object storage</Tooltip> đều có trên thiết bị.
...
Snow Family **không nạp trực tiếp** dữ liệu vào <Tooltip content="Lớp lưu trữ S3 giá rất rẻ dành cho dữ liệu archive ít truy cập, đổi lại thời gian lấy ra lâu hơn">S3 Glacier</Tooltip>. Dữ liệu luôn vào S3 (Standard) trước, sau đó bạn dùng một S3 Lifecycle rule để tự động chuyển nó sang Glacier.
...
- **Hai chiều và đa điểm:** chuyển dữ liệu qua giữa on-premises và AWS, giữa các regions/storage services trong AWS. Phía on-premises, bạn cài một DataSync agent (một máy ảo) để đọc dữ liệu qua <Tooltip content="Network File System — giao thức chia sẻ file phổ biến trên Linux/Unix">NFS</Tooltip> hoặc <Tooltip content="Server Message Block — giao thức chia sẻ file của Windows">SMB</Tooltip>.
...
Toàn cảnh kiến trúc: agent ở on-premises đọc dữ liệu qua NFS/SMB, mã hóa rồi truyền qua <Tooltip content="Transport Layer Security — giao thức mã hóa dữ liệu trên đường truyền, đảm bảo dữ liệu không bị đọc trộm khi đi qua internet">TLS</Tooltip> tới dịch vụ DataSync trong Region, rồi ghi xuống các đích lưu trữ — S3 (mọi storage class), EFS, hoặc FSx:
...
File system **Windows native** thật sự: dùng giao thức SMB, hệ thống file NTFS, tích hợp <Tooltip content="Active Directory — dịch vụ thư mục của Microsoft để quản lý người dùng, máy tính và quyền truy cập trong môi trường Windows">Active Directory</Tooltip> để phân quyền người dùng, kèm <Tooltip content="Access Control List — danh sách kiểm soát truy cập, quy định ai được đọc/ghi từng file hay thư mục">ACLs</Tooltip> và user quotas. Hỗ trợ các tính năng doanh nghiệp Windows như <Tooltip content="Distributed File System (DFS) Namespaces — gộp nhiều file system thành một không gian tên thống nhất, giúp người dùng truy cập qua một đường dẫn chung dù dữ liệu nằm rải rác">DFS Namespaces</Tooltip> để gom file trải trên nhiều file system.
...
- **Hai lựa chọn storage:** SSD cho workload nhạy với độ trễ (database, xử lý media, data analytics) và HDD cho workload phổ thông, rẻ hơn (home directory, <Tooltip content="Content Management System — hệ quản trị nội dung như WordPress, Drupal">CMS</Tooltip>).
- **Truy cập từ on-premises:** kết nối được từ hạ tầng on-premises qua <Tooltip content="VPN — đường hầm mã hóa qua internet công cộng để nối mạng on-premises với VPC">VPN</Tooltip> hoặc <Tooltip content="AWS Direct Connect — đường truyền vật lý riêng, ổn định, nối thẳng data center của bạn vào AWS, không đi qua internet">Direct Connect</Tooltip>.
...
**Lustre** là một file system song song, phân tán (parallel distributed) mã nguồn mở cho tính toán quy mô lớn — tên ghép từ "Linux" + "cluster". FSx for Lustre cho throughput tới hàng trăm GB/s, hàng triệu IOPS, độ trễ sub-millisecond — sinh ra cho các workload <Tooltip content="High Performance Computing — tính toán hiệu năng cao, dùng nhiều máy chạy song song cho mô phỏng khoa học, phân tích lớn, training ML">HPC</Tooltip>.
...
- **Truy cập từ on-premises:** dùng được từ server on-premises qua <Tooltip content="VPN — đường hầm mã hóa qua internet công cộng để nối mạng on-premises với VPC">VPN</Tooltip> hoặc <Tooltip content="AWS Direct Connect — đường truyền vật lý riêng, ổn định, nối thẳng data center của bạn vào AWS, không đi qua internet">Direct Connect</Tooltip>.

**Use case:** Machine Learning, HPC, xử lý video, mô hình tài chính, <Tooltip content="Electronic Design Automation — phần mềm thiết kế và mô phỏng vi mạch/chip, đòi hỏi I/O rất lớn">Electronic Design Automation (EDA)</Tooltip> — nói chung là các workload cần tốc độ I/O cực cao trên tập dữ liệu lớn.

### 3.3. FSx for NetApp ONTAP

**Vấn đề**: Mang workload ONTAP lên cloud.

<Tooltip content="NetApp ONTAP — hệ điều hành lưu trữ doanh nghiệp rất phổ biến, nhiều công ty đang chạy on-premises và muốn mang nguyên workload lên cloud">NetApp ONTAP</Tooltip> là một nền tảng storage doanh nghiệp dùng rộng rãi. FSx for NetApp ONTAP là phiên bản managed của nó trên AWS, nổi bật vì **đa giao thức**: hỗ trợ cùng lúc NFS, SMB và <Tooltip content="iSCSI — giao thức cho phép truy cập ổ đĩa khối (block) qua mạng IP, như thể đó là ổ cứng gắn trực tiếp">iSCSI</Tooltip>. Nhờ đó nó là dòng FSx tương thích rộng nhất.
...
**Use case:** di chuyển các workload đang chạy NetApp ONTAP hoặc <Tooltip content="Network Attached Storage — thiết bị lưu trữ gắn mạng, phục vụ file qua NFS/SMB cho nhiều máy trong mạng nội bộ">NAS</Tooltip> on-premises lên AWS mà gần như không phải đổi gì, hoặc cần một file system làm việc tốt với cả Linux lẫn Windows.
...
**Recovery point — chỗ dựa để khôi phục sau sự cố.** Volume Gateway định kỳ tạo ra một <Tooltip content="Recovery point — một bản chụp nhất quán của volume tại một thời điểm, lưu bền vững trên AWS; có thể dùng để tạo lại volume hoặc xuất thành EBS snapshot">recovery point</Tooltip> cho mỗi volume — một bản chụp nhất quán của volume tại một thời điểm, lưu bền vững trên AWS. Đây chính là chỗ dựa cho disaster recovery: dù thiết bị gateway on-premises có hỏng hoàn toàn, dữ liệu và các recovery point vẫn còn nguyên trên AWS.
...
Nhiều doanh nghiệp vẫn có quy trình backup bằng **băng từ (tape)**. Tape Gateway giả lập một <Tooltip content="Virtual Tape Library — thư viện băng từ ảo, mô phỏng các ổ băng và thư viện băng để phần mềm backup cũ tưởng như đang ghi ra băng thật">Virtual Tape Library (VTL)</Tooltip>: phần mềm backup hiện có cứ ghi ra "băng" như cũ, nhưng thực chất dữ liệu được lưu vào S3 và Glacier.
...
- Hỗ trợ ba giao thức: SFTP (<Tooltip content="SSH File Transfer Protocol — truyền file an toàn qua kênh SSH được mã hóa">SSH File Transfer Protocol</Tooltip>), FTPS (<Tooltip content="FTP over SSL/TLS — FTP có thêm lớp mã hóa">FTP có mã hóa TLS</Tooltip>), và FTP (không mã hóa, chỉ dùng trong mạng nội bộ).
