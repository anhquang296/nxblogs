...
**B-tree** (Balanced Tree — cây cân bằng) là một cấu trúc cây mà **mọi đường đi từ gốc xuống lá đều có cùng độ dài**. "Cân bằng" ở đây nghĩa là cây không bao giờ bị lệch một bên; thêm/xóa phần tử luôn giữ cho mọi lá ở cùng một độ sâu. Đây là tính chất nền tảng cho hiệu năng mà ta sẽ thấy ở mục 4.
...
Trong PostgreSQL, mỗi entry ở leaf node là một cặp **`(giá trị key, ctid)`**. **`ctid`** (Current Tuple ID) là địa chỉ vật lý của row trong heap, gồm hai phần: **`(block_number, item_offset)`**.
...
PostgreSQL mã hóa sự chênh lệch này bằng hai tham số chi phí: **`seq_page_cost`** (mặc định **1.0**) và **`random_page_cost`** (mặc định **4.0**). Nghĩa là theo ước lượng mặc định, đọc một page ngẫu nhiên "đắt" gấp **bốn lần** đọc một page tuần tự.
...
Index không phải "tạo xong là quên". Giống như bảng, index B-tree cũng **bloat** (phình to vì không gian lãng phí):

- **Page split** để lại các leaf page chỉ đầy một nửa.
- Khi row bị `DELETE`/`UPDATE`, entry index tương ứng trở thành **dead** nhưng chỉ được dọn khi `VACUUM` chạy. (Cơ chế dead tuple/MVCC được mổ xẻ trong [Deep Dive về Table Bloat](/posts/table-bloat).)

Index phình to nghĩa là cây cao hơn cần thiết, nhiều page hơn phải nạp, cache kém hiệu quả hơn. Vài thao tác bảo trì cốt lõi:
