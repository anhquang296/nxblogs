---
title: 'PostgreSQL Index: Vì Sao Index Tăng Tốc Query, Và Khi Nào Postgres Bỏ Qua Nó'
date: '2026-05-29'
tags: ['Database']
description: 'Mổ xẻ B-tree index trong PostgreSQL — từ cấu trúc cây, leaf node và ctid, sức mạnh duyệt cây logarit, đơn vị đọc page, đến cách planner cân nhắc random I/O để quyết định có dùng index hay không.'
enableComment: true
---

# PostgreSQL Index: Vì Sao Index Tăng Tốc Query, Và Khi Nào Postgres Bỏ Qua Nó
...
Một index được thiết kế theo cấu trúc bảng (ví dụ "cứ index mọi khóa ngoại") thường lãng phí; một index được thiết kế theo **query pattern thực tế** mới tạo ra khác biệt.
...
## 7. Index cũng bloat — một chút về bảo trì
...
- **Random I/O là gót chân Achilles.** Khi row khớp rải rác và đủ nhiều, planner bỏ index và chọn **Seq Scan** — quét tuần tự cả bảng một lần rẻ hơn hàng triệu lần fetch `ctid` ngẫu nhiên.
- **Thiết kế index theo cách ứng dụng query**, ưu tiên cột **cardinality cao** — những cột xuất hiện trong `WHERE`, `JOIN`, và `ORDER BY`.
