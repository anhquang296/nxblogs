...
<ExcalidrawDiagram
  ...
  alt="Các tầng cache trên AWS — CloudFront (edge cache), API Gateway (API cache), ElastiCache (Lazy Loading + Write-Through) với RDS/Aurora, và DAX với DynamoDB"
/>
...
<ExcalidrawDiagram
  ...
  alt="Luồng Cache-Aside / Lazy Loading — ứng dụng đọc cache, nếu miss thì query database rồi ghi ngược kết quả vào cache"
/>
...
<ExcalidrawDiagram
  ...
  alt="So sánh ba write strategy — Write-Through (ghi đồng bộ cả hai), Write-Back (ghi cache trước, flush DB sau), Write-Around (ghi thẳng DB, bỏ qua cache)"
/>
