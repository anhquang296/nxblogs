...
Trong VPC, resource ở <Tooltip content="Mạng con riêng — không có route trực tiếp ra internet">private subnet</Tooltip> chỉ có private IP. Internet không biết cách route đến private IP (thuộc dải RFC 1918 — dải địa chỉ IP dành riêng cho mạng nội bộ), nên các resource này **không thể giao tiếp trực tiếp với internet**.

<Tooltip content="Network Address Translation — dịch địa chỉ mạng">NAT</Tooltip> giải quyết vấn đề này bằng cách đặt một "đại diện" ở **public subnet**: khi EC2 trong private subnet muốn gọi ra internet, packet được gửi đến NAT, NAT thay source IP bằng IP public của mình rồi forward ra internet. Response quay về NAT, NAT tra bảng mapping rồi chuyển về đúng EC2 ban đầu.
...
NAT Instance thực chất là một **EC2 instance** chạy <Tooltip content="Amazon Machine Image — bản mẫu hệ điều hành">AMI</Tooltip> có cấu hình sẵn <Tooltip content="Cho phép OS chuyển tiếp packet giữa các network interface thay vì chỉ xử lý packet gửi đến chính nó">IP forwarding</Tooltip> và <Tooltip content="Công cụ quản lý firewall và NAT trên Linux">iptables</Tooltip>. Khi packet đến, thay vì xử lý ở tầng application, OS của EC2 này sẽ **thay đổi source IP** rồi **forward packet** ra interface khác — đúng như một router NAT truyền thống.
...
2. **Gắn <Tooltip content="Elastic IP — địa chỉ IP tĩnh public do AWS cung cấp">Elastic IP</Tooltip>** — để internet có thể gửi response về
3. **Disable Source/Destination Check** — chi tiết ở phần tiếp theo
4. **Cấu hình route table** — private subnet phải có route `0.0.0.0/0 → eni-xxx` trỏ đến <Tooltip content="Elastic Network Interface — card mạng ảo gắn vào EC2 instance trong VPC">ENI</Tooltip> của NAT Instance
...
Vì NAT Instance là EC2, bạn có thể (và phải) gắn <Tooltip content="Nhóm bảo mật — tường lửa ảo ở tầng instance, stateful">Security Group</Tooltip>. Đây vừa là **ưu điểm** (kiểm soát chi tiết traffic) vừa là **trách nhiệm** (cấu hình sai = mất kết nối).
...
> So sánh: NAT Gateway **không hỗ trợ Security Group** — bạn chỉ có thể kiểm soát traffic qua <Tooltip content="Network Access Control List — tường lửa ở tầng subnet, stateless">NACL</Tooltip>. Với NAT Instance, bạn có cả hai lớp bảo vệ: Security Group (stateful, ở tầng instance) + NACL (stateless, ở tầng subnet).
...
Theo truyền thống, NAT Gateway yêu cầu public subnet (subnet có route trực tiếp ra internet qua IGW) để truy cập internet. Khách hàng quan tâm bảo mật phải đảm bảo không ai tạo tài nguyên khác trong public subnet đó. Với Regional NAT Gateway, VPC **không cần public subnet** — đây là cải thiện bảo mật rất lớn vì VPC giờ có thể hoàn toàn private mà vẫn có <Tooltip content="Kết nối đi ra internet từ tài nguyên bên trong VPC">internet egress</Tooltip>.

**2. Tự động mở rộng theo AZ**

Regional NAT Gateway hoạt động ở cấp VPC. Khi bạn mở rộng workload sang AZ mới, bạn có thể tái sử dụng cùng một Regional NAT Gateway ID. Regional NAT Gateway tự động mở rộng và duy trì <Tooltip content="Ưu tiên xử lý traffic trong cùng AZ">zonal affinity</Tooltip> để đảm bảo high availability.
...
- Muốn dùng luôn làm <Tooltip content="Jump server — máy chủ trung gian để SSH vào các instance trong private subnet">bastion host</Tooltip>
