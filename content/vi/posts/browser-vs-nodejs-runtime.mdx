...
date: '2026-05-14'
tags: ['Javascript']
description: 'So sánh chi tiết Browser và Node.js runtime — JS engine, event loop, APIs khả dụng, và tại sao cùng một đoạn code lại chạy khác nhau.'
enableComment: true
---
...
<ExcalidrawDiagram
  ...
  alt="Kiến trúc runtime của browser và Node.js đặt cạnh nhau để so sánh"
/>
...
Dù engine nào — <Tooltip content="Javascript engine của Google, viết bằng C++, dùng trong Chrome và Node.js">V8</Tooltip>, SpiderMonkey, hay JavaScriptCore — tất cả đều đi qua pipeline giống nhau:

<ExcalidrawDiagram src="/browser-vs-nodejs-engine-pipeline.excalidraw" alt="Pipeline biên dịch của Javascript engine từ source code tới machine code" />

1. **Lexer/Tokenizer**: Tách source code thành các token (từ khoá, biến, toán tử)
2. **Parser**: Xây dựng <Tooltip content="Abstract Syntax Tree — cây cú pháp trừu tượng, là cấu trúc dữ liệu dạng cây mô tả logic của chương trình">AST</Tooltip>
3. **Bytecode Interpreter**: Thực thi bytecode ngay — startup nhanh, nhưng chưa tối ưu
4. **JIT Compiler**: Khi phát hiện code "hot" (chạy nhiều lần), <Tooltip content="Just-In-Time — biên dịch ngay tại thời điểm chạy">JIT</Tooltip> biên dịch trực tiếp thành machine code tối ưu
...
Câu trả lời: <Tooltip content="Vòng lặp điều phối tác vụ async trên single thread — lấy task từ queue, thực thi, rồi lặp lại">event loop</Tooltip> — một vòng lặp liên tục kiểm tra: "có việc gì cần làm không?" → thực thi → kiểm tra tiếp.
...
<ExcalidrawDiagram src="/browser-event-loop.excalidraw" alt="Kiến trúc event loop của browser qua macrotask, microtask và render" />
...
<a href="/tools/browser-event-loop-visualizer" target="_blank" className="inline-flex items-center gap-2 rounded-lg border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-sm font-medium text-blue-400 no-underline transition hover:bg-blue-500/20">▶ Mở Browser Event Loop Visualizer (interactive)</a>

### 2.2 Node.js Event Loop (libuv)

<ExcalidrawDiagram src="/nodejs-event-loop.excalidraw" alt="Event loop của Node.js với 6 phase của libuv chạy tuần tự" />

Node.js dùng thư viện C có tên <Tooltip content="Thư viện C đa nền tảng cung cấp async I/O, event loop, thread pool — nền tảng chạy của Node.js">libuv</Tooltip> để quản lý event loop. Thay vì một vòng đơn giản, libuv chia thành **6 phase** chạy tuần tự:
...
<a href="/tools/nodejs-event-loop-visualizer" target="_blank" className="inline-flex items-center gap-2 rounded-lg border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-sm font-medium text-blue-400 no-underline transition hover:bg-blue-500/20">▶ Mở Event Loop Architecture Visualizer (interactive)</a>
...
<Tooltip content="Web-interoperable Runtimes Community Group">WinterCG</Tooltip> là nỗ lực chuẩn hoá một tập API tối thiểu chung cho tất cả runtime: Node.js, Deno, Bun, Cloudflare Workers. Mục tiêu: viết một lần, chạy mọi nơi — ít nhất là cho phần không liên quan đến DOM hay file system.
