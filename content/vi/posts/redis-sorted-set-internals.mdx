...
<ExcalidrawDiagram src="/sorted-set-overview.excalidraw" alt="Sorted Set lưu các cặp member-score, tự sắp xếp theo score tăng dần" />
...
<ExcalidrawDiagram src="/sorted-set-hash-table.excalidraw" alt="Hash table ánh xạ member sang score, cho tra cứu O(1)" />
...
![Skip List nhiều tầng, tầng cao nhảy cóc qua nhiều node như tàu tốc hành](/sorted-set-skiplist.svg)
...
![Tìm score 2100 trên skip list, đi từ tầng cao xuống thấp qua 5 bước](/sorted-set-skiplist-search.svg)
...
![Trường span ghi số node bị nhảy qua, cộng dồn để tính ZRANK trong O(log N)](/sorted-set-span.svg)
...
<ExcalidrawDiagram src="/sorted-set-listpack.excalidraw" alt="Listpack lưu các cặp member-score liền kề trong khối bộ nhớ liên tục" />
...
<ExcalidrawDiagram src="/sorted-set-encoding-decision.excalidraw" alt="Lưu đồ chọn encoding listpack hay skiplist+hashtable theo hai ngưỡng" />
...
<ExcalidrawDiagram src="/sorted-set-dual-structure.excalidraw" alt="Hash table và skip list phối hợp, mỗi command đi tới cấu trúc phù hợp" />
